...
def print_folder_summary(
    ...
    *,
    ...
    since_ts = parse_date_ymd(filters.get('since')) if filters and filters.get('since') else None
    until_ts = parse_date_ymd(filters.get('until')) if filters and filters.get('until') else None
    ...
    def child_sort_key(n):
        if sort_key == 'size':
            ...
            val = (n.get('ts') or -1) if n['t'] == 0 else (max_ts.get(n['h']) if max_ts.get(n['h']) is not None else -1)
        else:
    ...
    def build_tree_and_print(n, depth=0):
        ...
        if n['t'] == 0:
            ...
            if not only_folders:
                ...
                print(f"{indent}- {name} (File{size_str}{ts_str})")
            return True
        ...
        print(f"{indent}- {name} (Folder - {format_size(folder_sz)}{ts_str})")
    ...
    if mbps:
        ...
        if roots:
            ...
            for r in roots:
                ...
                print(f"  {r['dec_name']}: {format_size(sz)}  →  ~{format_duration(eta) if eta is not None else 'n/a'}")
...
if __name__ == "__main__":
    ...
    # CHANGED: -of (and keep -OF for compatibility), plus --only-folders
    ...
    parser.add_argument('--until', help="Only files with timestamp <= this date (YYYY-MM-DD).")
    # Sorting
